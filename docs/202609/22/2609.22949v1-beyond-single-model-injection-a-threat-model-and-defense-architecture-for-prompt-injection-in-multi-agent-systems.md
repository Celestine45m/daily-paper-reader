...
tags: ["query:as"]
score: 7.0
evidence: 多智能体系统中提示注入的威胁模型与防御
tldr: "现有提示注入研究多聚焦单模型聊天场景，但多智能体系统通过智能体间消息传递、共享工具权限与信任传播放大威胁。本文构建涵盖14种攻击向量的威胁模型，并在6智能体生产级系统上验证。结果显示，即便有系统提示级护栏，67%的智能体仍易受至少一种越权攻击，工具输出间接注入成功率达43%。四项架构防御将整体注入成功率从31.2%降至4.2%，为多智能体安全提供威胁模型与防御架构。"
source: arxiv
...
motivation: 现有提示注入研究局限于单模型聊天，忽视多智能体系统中消息传递、共享工具与信任传播带来的放大风险。
method: 构建四类14种攻击向量的威胁模型，在6智能体生产级系统上测试，并设计消息签名、边界净化、权限限定与异常检测四类防御。
result: "测试显示67%智能体越权，工具输出间接注入成功率43%；四项防御使整体注入成功率从31.2%降至4.2%，消息签名令智能体间注入降91%。"
conclusion: 该工作揭示多智能体提示注入的独特风险，提出的威胁模型与四层架构防御显著提升系统安全性。
---

## 摘要
现有提示注入研究主要关注单模型聊天机器人场景，其中攻击者通过精心构造的输入操纵单个LLM。多智能体系统通过单模型设置中不存在的三种机制放大了这一威胁：智能体间消息传递创建了边界防御不可见的注入通道，共享工具访问使权限提升能够跨越智能体边界，而信任传播允许被攻陷的智能体影响上游编排器。我们构建了一个威胁模型，列举了四类共14种攻击向量：通过用户输入的直接注入（3种向量）、通过工具输出的间接注入（4种向量）、通过消息传递的智能体间注入（4种向量），以及通过编排器操纵的级联注入（3种向量）。在一个具有生产代表性的6智能体系统上测试全部14种向量，我们发现，即使有系统提示级别的防护措施，67%的智能体仍易受至少一种范围违规的影响，并且通过工具输出的间接注入在43%的尝试中成功。四种架构防御将总体注入成功率从31.2%降至4.2%：带有来源追踪的消息签名（智能体间注入下降91%）、智能体边界处的输入/输出净化（间接注入下降78%）、按智能体角色限定权限的工具访问（权限提升被完全消除），以及智能体间通信模式上的异常检测（捕获84%的级联尝试）。
